...
public class Demo {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// Connect to my UTA Omega Oracle database, putting my account and password in
			// the parameter
			// Register Oracle Driver and Create Statement for using SQL in future
			DriverManager.registerDriver(new OracleDriver());
			Connection cn = DriverManager.getConnection(
					"jdbc:oracle:thin:@//acaddbprod-1.uta.edu:1523/pcse1p.data.uta.edu", "yxz0700", "Yingying8825");
			Statement st = cn.createStatement();
			System.out.println("Connection Set Up");

			// Load the records from my computer to the UTA database, using for loop to
			// handle the path
			...
			for (String filename : files) {
				String path = "C:\\Users\\zhaoy\\OneDrive\\Desktop\\uta\\2021 spring\\5330 Database\\hw1\\DataFiles-1\\"
						+ filename + ".txt";
				BufferedReader br = new BufferedReader(new FileReader(path));
				String line = null;
				while ((line = br.readLine()) != null) {

					// Replace the filename and line every time, the insert SQL generate easily and
					// correctly.
			...
			// To avoid circular reference, insert the records first, and add foreign key in
			// the end.
			// By doing this, we avoid to add foreign key in the table, and then set foreign
			// key to null, renew the foreign key in the end.
